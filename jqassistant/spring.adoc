...
[source,cypher,role=concept,requiresConcepts="spring-component:VirtualDependency",reportType="plantuml-class-diagram"]
.Actual usage dependencies between Spring components, i.e. controller, service and repository implementations.
----
MATCH
  (package:Package)-[:CONTAINS]->(component:Spring:Component)
OPTIONAL MATCH
  (:Main:Artifact)-[:CONTAINS]->(superType:Type),
  (component)-[e:EXTENDS|IMPLEMENTS*]->(superType)
OPTIONAL MATCH
  (component)-[:DEPENDS_ON]->(dependency:Spring:Component)
RETURN
  *
----
